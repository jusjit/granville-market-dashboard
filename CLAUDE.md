...
- **Panel layout (updated 2026-08-01)**: Daily centroid card (violet, centroid value + pivots/targets + sigma bands inside) → `AlmaLiveCard` (live SPX/VIX reference — SPX Last, Prev Close, Open, Gap from Centroid, VIX Gap) → Weekly levels card. Centroid is the first thing visible when opening the section. Active Rules (`AlmaActiveRules`, named export) are rendered in App.jsx AFTER AlmaLog, not inside AlmaPanel. `AlmaLiveCard` polls `/api/alma` on its own 15-min `setInterval` in `App.jsx`, independent of the manual Refresh button (cheap: Supabase + a couple Tradier calls, no LLM — unlike the rest of the dashboard which stays manual-only to control 1min.ai/Finnhub cost).
...
- **Event calendar (dynamic, replaces hardcoded 2026 array)**: `api/tradier.js` fetches events from three sources in parallel alongside the vol surface computation and returns them as an `events` array in the response. `annotateEvents()` (client-side, `src/lib/volsurface.js`) assigns events to the expiry window they fall in.
  - **FRED** (7 releases): CPI (10), NFP (50), Retail Sales (9), Personal Income/Spending/Core PCE (54), Michigan Sentiment (91), JOLTS (192), Existing Home Sales (291) via `fred/release/dates`. Uses existing `FRED_KEY`. Labels abbreviated for x-axis: CPI, NFP, Retail, PCE, UMich, JOLTS, Homes.
  - **Finnhub**: `/calendar/earnings` filtered to 20 SPX mega-caps (`EARNINGS_TICKERS` set: AAPL, MSFT, AMZN, GOOGL, META, NVDA, TSLA, etc.). Uses existing `FINNHUB_KEY`. Free tier returns ~1500 entries/month — ample coverage.
  - **Hardcoded calendars** (3 arrays in `api/tradier.js`, update annually):
    - `FOMC_DATES` — FRED has no clean release for FOMC decision days (release_id=21 is Minutes, 101 is daily). 2026+2027 included.
    - `ISM_MFG_DATES` — ISM Manufacturing PMI, 1st business day of month. ISM not in FRED. Source: ismworld.org/supply-management-news-and-reports/reports/rob-report-calendar/
    - `ISM_SVC_DATES` — ISM Services PMI, 3rd business day of month. Same source.
...
- **History = OVERLAY, not replace**: `snapshot-vol.js` (Bearer SNAPSHOT_SECRET) writes `vol_surface_snapshots` every 2h RTH via `.github/workflows/vol-snapshot.yml`; `vol-history.js` lists recent snapshots. VolSurfacePanel "Compare snapshot" toggle keeps the LIVE curve and draws a chosen past snapshot's spot IV AND forward IV as ghosted dashed lines (merged by expiration; rolled-off expiries show on one series only; tooltip shows Δ for both spot and forward IV). Slider appears only with ≥2 snapshots. ⚠️ DST: bump vol-snapshot crons +1h UTC after Nov 1 2026.
  - **Forward IV in snapshots**: each point in the `points` JSONB already stores `forwardIV` (variance-additive computation between adjacent expiries). As of 2026-07-22, also stores `forwardNearExp`/`forwardFarExp` (the specific expiry pair used — lets you distinguish IV shift vs pair roll) and `ivGap` (`forwardIV - spotIV`). Old snapshots have `null` for the new fields but retain `forwardIV` — no backfill needed, no schema migration (all inside existing JSONB column).
...
1. **World Briefing** (added 2026-08-02, geo map 2026-08-28) — Interactive SVG
   ...
   below the map; elevated/watch regions are click-to-expand. ViewBox 960×480
   for mobile readability. Below the map: Minto pyramid from LLM `briefing`
