...
- **Panel layout (updated 2026-08-01)**: Daily centroid card (violet, centroid value + pivots/targets) → `AlmaLiveCard` (live SPX/VIX reference — SPX Last, Prev Close, Open, Gap from Centroid, VIX Gap) → Active Rules → Sigma Bands → Weekly levels card. Centroid is the first thing visible when opening the section. `AlmaLiveCard` polls `/api/alma` on its own 15-min `setInterval` in `App.jsx`, independent of the manual Refresh button (cheap: Supabase + a couple Tradier calls, no LLM — unlike the rest of the dashboard which stays manual-only to control 1min.ai/Finnhub cost).
...
1. **Alma Centroid** — private dashboard only (`VITE_SHOW_ALMA=true`). Layout: Daily centroid card (violet, with pivots/targets) → Live SPX Reference (SPX Last, Prev Close, Open, Gap from Centroid, VIX Gap) → Active Rules → Sigma Bands → Weekly levels card.
