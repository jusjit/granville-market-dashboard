...
COUNTER-EVIDENCE REQUIREMENT: For each escalating or critical theme, you MUST actively search the provided data for evidence that would suggest de-escalation, resolution, or a less severe read than the primary assessment — even if that counter-evidence is less prominent than the escalation signals. Report this as a "counter_signals" field per theme. Look for: diplomatic talks, ceasefire signals, partial flow normalization, bypass capacity, troop withdrawals, severity downgrades, falling incident counts, or any trend that cuts against the escalation narrative. If no genuine counter-evidence exists for a given theme, explicitly state "No meaningful de-escalation signals found in current data" — do NOT leave the field empty or omit it.

Output strict JSON: {flagged: boolean, risk_category: string, confidence: INTEGER 0-100 (NOT a 0-1 probability — e.g. 65, not 0.65), transmission_chain: string, relevant_signals: string[], granville_modifier: {position_size_cap: number between 0 and 1, alma_reversion_confidence: INTEGER 0-100 (NOT a 0-1 probability — e.g. 28, not 0.28)}, categories_considered: string[], categories_dismissed_reason: {<category>: "one-line reason not flagged"}, briefing: {bottom_line: "one sentence — the single most important situational takeaway right now, pure geopolitical, NO market-implication language", themes: [{region: "geographic area or corridor", situation: "2-3 sentence plain-language assessment of what is happening and why it matters", sources_confirmed: ["which data sources corroborate this"], counter_signals: "1-2 sentence summary of any de-escalation evidence, diplomatic progress, flow normalization, or other signals that cut against the escalation narrative. If none found: 'No meaningful de-escalation signals found in current data.'"}]}}`
...
const GDELT_QUERY_BATCHES = [
  ...
  '(OPEC OR "nord stream" OR pipeline OR "lng export" OR nuclear OR "ballistic missile")',
  '(NATO OR ukraine OR "black sea" OR crimea OR coup OR "martial law")',
  '("regime change" OR "cyber attack" OR "critical infrastructure" OR "rare earth" OR TSMC OR semiconductor)',
]
